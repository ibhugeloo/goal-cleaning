...
export function FAQ() {
    ...
    return (
        <section id="faq" className="py-16 md:py-24 bg-[#F5F0E1]">
            <div className="container mx-auto px-4 max-w-3xl">
                <div className="text-center mb-16">
                    <h2 className="font-display text-3xl md:text-5xl font-extrabold tracking-tight mb-4">
                        <span className="text-[#003631]/60">Questions </span>
                        <span className="text-[#003631]">Fréquentes</span>
                    </h2>
                    <p className="text-[#003631]/50 text-lg">
                ...
                <Accordion type="single" collapsible className="w-full">
                    {faqs.map((faq, index) => (
                        <AnimateInView key={index} delay={index * 100}>
                            <AccordionItem value={`item-${index}`} className="border-[#003631]/15">
                                <AccordionTrigger className="text-base font-medium text-[#003631]/80 hover:text-[#003631] transition-colors [&>svg]:text-[#003631]/50">
                                    {faq.question}
                                </AccordionTrigger>
                                <AccordionContent className="text-[#003631]/55 leading-relaxed">
                                    {faq.answer}
                                </AccordionContent>
                            </AccordionItem>
                        </AnimateInView>
